<?php 
class Comment_author extends CI_Controller{
	...
	function index(){
        ...
        $this->load->view('layout/back_n/author/v_comment_author');
        $this->load->view('partials/back_n/author/footer');
        $this->load->view('partials/back_n/js/js_comment');
	}
	...
	function upload_image(){
		if(isset($_FILES["file"]["name"])){
			 $config['upload_path'] = 'upload/comment/';
			 ...
			 if(!$this->upload->do_upload('file')){
		            ...
		            $config['source_image']='upload/comment/'.$data['file_name'];
		            $config['create_thumb']= FALSE;
		            $config['maintain_ratio']= TRUE;
		            $config['quality']= '60%';
		            $config['width']= 600;
		            $config['height']= 488;
		            $config['new_image']= 'upload/comment/'.$data['file_name'];
		            $this->load->library('image_lib', $config);
		            $this->image_lib->resize();
					echo base_url().'upload/comment'.$data['file_name'];
			 }
		}
	}
	...
	function reply(){
		...
		if($query->num_rows() > 0){
			...
			redirect('index.php/back_n/author/comment_author');
		}else{
	...
	function unpublish(){
		...
		$this->load->view('partials/back_n/author/header',$data);
		$this->load->view('partials/back_n/author/sidebar');
        $this->load->view('layout/back_n/author/v_comment_unpublish_author');
        $this->load->view('partials/back_n/author/footer');
        $this->load->view('partials/back_n/js/js_comment_unpublish');
	}
}